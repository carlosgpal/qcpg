...
public interface GraphAnalysisRepository extends Neo4jRepository<GenericNode, Long> {
  ...
  @Query("""
      ...
      MATCH path = (startOp)-[:EXECUTION_ORDER*]->(op)
  ...
  @Query("""
            MATCH (cn:QUANTUM_GATE_CX {file: $file})
      MATCH (cn)-[:QUANTUM_OPERAND_0|QUANTUM_OPERAND_1]->(qb:QUANTUM_BIT)
      WITH collect(DISTINCT qb) AS cnotQubits
      MATCH (allQ:QUANTUM_BIT {file: $file})
      WITH cnotQubits, collect(DISTINCT allQ) AS allQList
      RETURN CASE WHEN size(allQList) = 0 THEN 0.0
            ELSE 100.0 * size(cnotQubits) / size(allQList)
             END AS percentage
                  """)
  ...
  @Query("""
        MATCH (q:QUANTUM_BIT {file: $file})
      OPTIONAL MATCH (q)<-[:QUANTUM_OPERAND_0|QUANTUM_OPERAND_1]-(cn:QUANTUM_GATE_CX {file: $file})
      WITH q, count(cn) AS numCnot
      RETURN avg(numCnot) AS avgCNOT
        """)
  ...
  @Query("""
                  MATCH (toff:QUANTUM_GATE_CCX {file: $file})
      MATCH (toff)-[:QUANTUM_OPERAND_0|QUANTUM_OPERAND_1|QUANTUM_OPERAND_2]->(qb:QUANTUM_BIT)
      WITH collect(DISTINCT qb) AS toffQubits
      MATCH (allQ:QUANTUM_BIT {file: $file})
      WITH toffQubits, collect(DISTINCT allQ) AS allQList
      RETURN CASE WHEN size(allQList) = 0 THEN 0.0
            ELSE 100.0 * size(toffQubits) / size(allQList)
             END AS percentage
                  """)
  ...
  @Query("""
            MATCH (q:QUANTUM_BIT {file: $file})
      OPTIONAL MATCH (q)<-[:QUANTUM_OPERAND_0|QUANTUM_OPERAND_1|QUANTUM_OPERAND_2]-(toff:QUANTUM_GATE_CCX {file: $file})
      WITH q, count(toff) AS numToff
      RETURN max(numToff) AS maxToff
            """)
  ...
  @Query("""
      ...
      WITH sum(CASE WHEN qbits = 1 THEN 1 ELSE 0 END) AS singleQ,
           count(g) AS total
      RETURN 100.0 * singleQ/total AS percentage
        """)
